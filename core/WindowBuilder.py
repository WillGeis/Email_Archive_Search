...
from tkinter import *
from PDFOCR import OCR_folder

## Head window for selecting PDF extraction, email search, or OCR
def selection_window():
    ...
    Button(master, text='PDF OCR', command=on_OCR_window, width=50).grid(row=3, column=0, pady=5)

    master.mainloop()


## OCR function for a single PDF
def OCR(pdf_path, output_folder):
    ...
    for i, image in enumerate(images):
        image_path = os.path.join(output_folder, f"temp_page_{i+1}.jpg")
...
def OCR_window():
    ...
    ocr = Tk()
    ocr.title("OCR Window")
    ocr.geometry("800x600")

    Label(ocr, text="Please enter path to PDF archive").grid(row=0, column=0, pady=10, padx=10)
    path_entry = Entry(ocr, width=50)
    path_entry.grid(row=0, column=1, pady=10, padx=10)

    Button(ocr, text="Run OCR", command=start_ocr, width=30).grid(row=1, column=0, columnspan=2, pady=20)
